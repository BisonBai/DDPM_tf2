...
num_epochs = 10  # train for at least 100 epochs for good results 
...
class FID(keras.metrics.Metric):
    ...
    # calculate activations
        ...
        mu1 = tf.math.reduce_mean(act1 , axis =0 )
        mu2 = tf.math.reduce_mean(act2, axis =0 )
        ...
        ssdiff = tf.math.reduce_sum(tf.math.square(tf.math.subtract(mu1, mu2)))
        

        covmean =  tf.math.sqrt(tf.tensordot(sigma1, sigma2 , 1))
        
        covmean = tf.math.real(covmean)
        

        sigma_sum = tf.math.add(sigma1 , sigma2 )
        
        prod = tf.math.add(sigma_sum , tf.math.multiply(-2.0, covmean))
        
        fid = tf.math.add(ssdiff , tf.linalg.trace(prod))
...
class DiffusionModel(keras.Model):
    ...
    def reverse_diffusion(self, initial_noise, diffusion_steps):
        ...
        for step in range(diffusion_steps, 0, -1):  # t = T,...,1
            ...
            #print("Diffusion times :{}".format(diffusion_times)) ## V
    ...
    def train_step(self, images):
        ...
        # KID, FID  is not measured during the training phase for computational efficiency
        return {m.name: m.result() for m in self.metrics[:-2]} ## 3/4 V
